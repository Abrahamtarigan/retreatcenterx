<html>
    ...
    <br/>
    <table id="transaksi_clear" class="table table-striped" >
      <thead>
        <tr>
      ...
      <tbody>
        <?php $i = 1; ?>
        <?php foreach ($getSuccessTransactions as $gas) { ?>
          <?php
          date_default_timezone_set('Asia/Jakarta');
          $current_date = date('Y-m-d H:i:s');

            $tgl_check_in = date_create($gas->bookingStDt);
            $tgl_check_out = date_create($gas->bookingEdDt);
            $time_booking = date_create($gas->bookingTime);
            $end_period = date_create($gas->bookingTimeStacEnd);
          ...
          ?>
          <tr>
            <th><a style="font-weight:bold"><?= $gas->roomName; ?></a>&nbsp;<br/>by:
              <a style="font-weight:bold"><a class="text-primary"><?= $gas->userNama; ?></a></a><br />
              <small>Id Booking : <a style="font-weight:bold"> <?= $gas->bookingId_text; ?></small></a><br/>
            ...
            <?= number_format($gas->bookingTotalPay, 0, ".", ".")?></small></a>
              
            </th>

            
            <td style="width:200px;">
              <?php if ($gas->status_booking == 2 )
              {
                ?>
                  <div class="alert alert-success" role="alert">
                    <?= $gas->status_booking_name; ?>
                   </div>
                <?php
              }else if($gas->status_booking_name == 1 )
              {
                ?>
              ...
              <?php
              }else if($gas->status_booking_name == 0 ){
                ?>
